...
<head>
    ...
    <style type="text/css">
        ...
        .session-color-box-under:after {
            ...
            transform: rotate(22deg) translate(.9rem, -.3rem);
...
<section class="overview">
    ...
    <?php foreach($cartBySubject as $subjectCode => $bySubject): ?>
    <?php if ($breakPage): ?>
        <div style="display: block; width: 100%; page-break-before: always;"></div>
    <?php else: $breakPage = true; ?>
    <?php endif; ?>
        <?php $color = '#' . current(array_filter($subjects, function($subject) use ($subjectCode) {return strtolower($subject['subject_code']) === strtolower($subjectCode);}))['session_color'];
        if ($color === null) $color = 'black'; ?>
        <div class="container" >
            <h1 style="position: relative;"><?= __d('admin-orderdrop', 'ORDER_ID'); ?>: <?= $order['Order']['id']; ?>
                - <?= $this->Time->format($order['Order']['modified'], Configure::read('Formats.datetime')); ?> - <?= ++$counter; ?>/<?= count($cartBySubject); ?>
                - <?= current(array_filter($subjects, function($subject) use ($subjectCode) {return strtolower($subject['subject_code']) === strtolower($subjectCode);}))['session_name']; ?>
            ...
            <div style="float: left; width: 45%; margin-right: 5px;">
                ...
                <section>
                    ...
                    foreach ($subjects as $subject): if (strtolower($subject['subject_code']) === strtolower($subjectCode)): ?>
                ...
                <div>
                    ...
                    <table class="transaction-table">
                        ...
                        <tr>
                            ...
                            <td><?= !empty($lastTransaction['Transaction']) ? $lastTransaction['Transaction']['transaction_code'] : ''; ?></td>
                        </tr>
                        <tr>
                            <td><?= __d('admin-orderdrop', 'TRANSACTION_TYPE'); ?>:</td>
                            <td><?= !empty($lastTransaction['Transaction']) ? $lastTransaction['Transaction']['payment_module'] : ''; ?></td>
                        </tr>
                    </table>
            ...
            <div class="cart">
                <?php $subject = array_filter($subjects, function($subject) use ($subjectCode) {return strtolower($subject['subject_code']) === strtolower($subjectCode) || isset($subject[$subjectCode]['subject_info']['groupe']);}); ?>
                    ...
                    <div class="cart-item" style="page-break-inside: avoid; margin-bottom: 0.125in;display: grid; grid-auto-columns: 1fr; gap: 0 0.125in">
                        <div style="grid-column:1/2; white-space:normal;">
                            <h3 style="max-width: 15ch; margin: 0 0 1rem;">
                                <?= $products[$item['productId']]['name_locale']; ?>
                            ...
                            <ul>
                                <li><?= __d('admin-orderdrop', 'UNIT_SUBTOTAL'); ?>: <?= $this->Number->currency($item['subtotal']); ?></li>
