 ...
 */
	public function write($type, $message, $options) {
		$output = '';
		if (empty($options['skipPrepend'])) {
			$output = date('Y-m-d H:i:s') . ' ' . ucfirst($type) . ': ';
		}
		$output .= $message . (empty($options['skipNewline']) ? chr(10) : '');
